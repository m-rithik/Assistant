...
// Import captcha solver with error handling
let solveCaptchaFromBase64 = null;
try {
  const captchaModule = await import('../../../../lib/captcha/nodeSolver.js');
  solveCaptchaFromBase64 = captchaModule.solveCaptchaFromBase64;
  ...
  console.error('❌ Failed to load captcha solver:', error.message);
  console.log('⚠ Captcha solver not available, manual captcha will be required');
}
...
const fetchCaptchaImage = async (cookieJar) => {
  try {
    ...
    const captchaUrl = `${BASE_URL}/vtop/get/new/captcha`;
...
const vtopLogin = async (username, password, retryCount = 0) => {
  ...
  if (captchaSrc) {
    ...
    try {
      ...
      if (retryCount < MAX_RETRIES - 1) {
        ...
        return vtopLogin(username, password, retryCount + 1);
      }
    ...
    if (captchaAnswer) {
      console.warn('⚠ User provided captcha but none was on page - ignoring it');
      solvedCaptcha = ''; // Clear it
    }
  }
...
export async function GET(req) {
  try {
    ...
    console.error("VTOP login GET error:", error);
    return NextResponse.json({
      success: false,
      error: error.message || "Failed to check VTOP session",
      timestamp: new Date().toISOString()
    }, { status: 500 });
  }
}
